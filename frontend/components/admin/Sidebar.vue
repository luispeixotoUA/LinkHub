<template>
  <div
    class="h-screen flex flex-col bg-gradient-to-b from-gray-800 to-gray-900 text-white w-64 flex-shrink-0 transition-all duration-300 border-r border-gray-700"
  ...
  >
    ...
    <nav class="flex-1 py-4 overflow-y-auto">
      ...
      >
        <span class="text-xl min-w-[24px]" :title="item.name">{{
          item.icon
        }}</span>
        <span
...
const menuItems = computed(() => {
  if (isAdmin.value) {
    return [{ name: "Utilizadores", path: "/admin/admin", icon: "👥" }];
  }
  return [
